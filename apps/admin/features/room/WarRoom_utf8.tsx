...
import {
  X, Mic, MicOff, Send, ArrowLeft, Hand, UserPlus, Clock as ClockIcon, Search, PhoneOff, Check, Ban, Headphones, VolumeX, Users, Trash2, ShieldAlert, ShieldOff, Link, Share2, Paperclip, Lock
} from "lucide-react";
...
export function WarRoom({ roomId, user, onExit }: WarRoomProps) {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (roomError || !room) { toast.error("Sala n├úo encontrada"); onExit(); return; }
      
      // --- VERIFICA├ç├âO AT├öMICA DE SALA ZUMBI ---
      ...
      if (endTime.isBefore(moment()) || room.status === 'ended') {
        ...
        toast.error("Este momento de clamor j├í foi encerrado. ­ƒÖÅ");
        onExit();
        return;
      }
      ...
      // --- TRAVA DE SEGURAN├çA NUCLEAR PARA SALAS PRIVADAS ---
      if (room.visibility === 'private' && !isCreator) {
        ...
        if (!participation) {
          await supabase.from('system_errors').insert({
            ...
            error_message: `Acesso NEGADO: Usu├írio ${user.id} tentou entrar na Sala Privada ${roomId}`,
            severity: 'high'
          });
          toast.error("Este clamor ├® privado. Voc├¬ precisa de um convite para entrar. ­ƒÖÅ");
          onExit();
          return;
        }
      }
      ...
      if (isCreator) {
        ...
        // Vamos apenas garantir que ele esteja l├í.
  ...
  if (loading || !token) {
    return (
      <div className="fixed inset-0 z-50 bg-[#0e0e0e] flex flex-col items-center justify-center gap-6">
        ...
        <p className="text-[#3fff8b] font-black uppercase tracking-[0.4em] text-[10px] animate-pulse">Sintonizando Canal de Ora├º├úo...</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <RoomAudioRenderer />
      <StartAudio 
        label="­ƒöè CLIQUE PARA OUVIR O CLAMOR" 
...
function WarRoomInterface({ roomData, setRoomData, user, onExit }: { roomData: any; setRoomData: React.Dispatch<any>; user: any; onExit: () => void }) {
  ...
  useEffect(() => {
    if (message) {
      try {
        ...
        console.error("Erro ao processar rea├º├úo:", e);
      }
    }
  }, [message]);
    ...
    const tInterval = setInterval(async () => {
      ...
      if (diff <= 0) {
        ...
        if (roomData.creator_id === user.id && roomData.status !== 'ended') {
          ...
          toast.error("O tempo do clamor acabou! ­ƒÖÅ");
    ...
    const sc = supabase.channel(`war-room-control-${roomData.id}`)
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'requests', filter: `room_id=eq.${roomData.id}` }, async (p) => {
        ...
        if ((p.new as any).user_id === user.id && (p.new as any).status === 'approved') {
          setMyRole('speaker');
          toast.success("Seu microfone foi liberado! ­ƒÄñ");
          ...
          setTimeout(async () => {
            if (localParticipant) {
              try {
                ...
                console.error("Erro na ativa├º├úo p├│s-aprova├º├úo:", e);
              }
            }
          }, 600);
        }
  ...
  const handleSendMessage = async (content: string, file?: File) => {
    ...
    if (file) {
      ...
      let fileExt = file.name?.split('.').pop() || (isImage ? 'jpg' : file.type.split('/')[1]) || 'bin';
      ...
      if (isImage) {
        ...
        fileExt = contentType.split('/')[1] || 'webp';
      }

      const fileName = `chat_${Date.now()}_${user.id}.${fileExt}`;
      const { data, error: uploadError } = await supabase.storage
        .from('avatars')
        .upload(fileName, finalFile, { contentType: contentType });
  ...
  const approveReq = async (r: any) => {
    ...
    toast.success("Intercessor aprovado! ­ƒÄñ");
  };
  ...
  useEffect(() => {
    ...
    const enableMic = async () => {
      try {
        ...
        console.error("ÔØî Erro ao controlar microfone:", err);
      }
    };
    enableMic();
  ...
  const handleEndRoom = async () => {
    ...
    toast.success("Sala de Guerra encerrada com sucesso! ­ƒÖÅ");
    onExit();
  };
  ...
  return (
    <div className="relative flex h-screen w-full flex-col overflow-hidden bg-[#0a0a0a] text-white font-body antialiased">
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[300] w-full max-w-[280px] flex flex-col gap-4 pointer-events-none">
        {!canPlayAudio && (
          ...
          >
            ...
            <div>
              <p className="text-[10px] font-black uppercase text-white tracking-widest">Seu ├íudio est├í travado</p>
      ...
      <div className="flex items-center justify-between px-6 pb-4 pt-[env(safe-area-inset-top,1rem)] z-20">
        <button onClick={handleLeaveSilently} className="flex items-center justify-center size-10 rounded-full hover:bg-white/10 transition-colors">
          <ArrowLeft size={20} />
        </button>
        <div className="flex flex-col items-center">
          <h1 className="font-headline font-extrabold text-sm tracking-tight uppercase text-center text-white">
            {roomData?.name || "Intercess├úo Sagrada"}
          </h1>
          <div className="flex items-center gap-1.5 mt-0.5">
            ...
            {roomData?.visibility === 'private' ? (
              ...
              <div className="flex items-center gap-1 text-gray-500">
                <Users size={10} />
                <span className="text-[9px] font-black uppercase">P├║blica</span>
              </div>
            )}
          </div>
        </div>
        <button onClick={handleCloseClick} className="flex items-center justify-center size-11 rounded-full hover:bg-white/10 transition-all active:scale-90 text-white">
          <X size={24} />
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center pt-2 px-6 overflow-hidden relative">
          ...
          <div className="absolute right-4 flex flex-col gap-3">
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  toast.info(newState ? "­ƒÄñ Microfone ATIVADO" : "­ƒöç Microfone MUTADO");
                } catch (err) {
            ...
            {myRole === 'creator' || myRole === 'admin' || myRole === 'speaker' ? (
              <button 
                ...
                className="size-12 rounded-full glass-panel flex items-center justify-center border border-[#3fff8b]/30 bg-[#3fff8b]/10 text-[#3fff8b] transition-all active:scale-95"
              >
                <Headphones size={20} />
              </button>
            ) : (
            ...
            <button 
              onClick={() => {
                const shareData = {
                  title: roomData?.name || "War Room F├®Conecta",
                  text: `Venha participar deste clamor conosco! ­ƒÖÅ ${roomData?.name || ''}`,
                ...
                if (navigator.share) {
                  ...
                  toast.success("Link de intercess├úo copiado! ­ƒÖÅ");
                }
              }}
        ...
        <div className="w-full max-w-[320px] flex flex-col gap-1.5 mb-1 shrink-0">
          <div className="flex gap-3">
            <button onClick={() => { handleAddReaction("­ƒÖÅ"); handleSendMessage("Am├®m! ­ƒÖÅ"); }} className="primary-gradient flex-1 h-12 rounded-full flex items-center justify-center shadow-xl active:scale-95 transition-all">
              <span className="font-headline font-extrabold text-[#0e0e0e] text-xs tracking-widest uppercase italic">Am├®m</span>
            </button>
            <button onClick={() => { handleAddReaction("­ƒÖî"); handleSendMessage("Gl├│ria! ­ƒÖî"); }} className="primary-gradient flex-1 h-12 rounded-full flex items-center justify-center shadow-xl active:scale-95 transition-all">
              <span className="font-headline font-extrabold text-[#0e0e0e] text-xs tracking-widest uppercase italic">Gl├│ria</span>
            </button>
          </div>
          <div className="flex justify-between px-4">
            {["­ƒÖÅ", "­ƒæÅ", "­ƒöÑ", "ÔØñ´©Å"].map((e) => (
        ...
        <div className="relative flex items-center gap-2">
          ...
          <div className="flex-1 relative">
            <input
              ...
              placeholder="Envie sua intercess├úo..."
        ...
        <div className="fixed inset-0 z-[3000] bg-black/90 backdrop-blur-xl flex items-center justify-center p-6">
          <div className="w-full max-w-sm bg-[#131313] rounded-[2rem] p-8 flex flex-col items-center gap-6 border border-white/5 text-center shadow-2xl">
            ...
            <div>
              ...
              <p className="text-white/60 text-xs font-medium">Voc├¬ ├® o criador desta sala. O que deseja fazer?</p>
            </div>
