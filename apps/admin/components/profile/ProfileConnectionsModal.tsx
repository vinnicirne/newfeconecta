            ...
            data.map((person) => (
              <div key={person.id} className="flex items-center justify-between p-3 hover:bg-gray-50 dark:hover:bg-white/5 rounded-2xl transition-all group">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-2xl overflow-hidden border border-gray-100 dark:border-white/10 relative">
                    {person.avatar_url && !person.avatar_url.includes('vercel.sh') && !person.avatar_url.includes('shadcn.png') ? (
                      <Image 
                      ...
                      <div className="w-full h-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-black text-lg uppercase shadow-inner">
                        {(() => {
                          const name = person.full_name || person.username || "U";
