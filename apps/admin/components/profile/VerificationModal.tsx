...
export function VerificationModal({ isOpen, onClose, user, onRequested }: any) {
  ...
  const [prices, setPrices] = useState<any>({
    ...
    "Igreja": "14,99", "Evangelista": "6,99", "Diácono": "6,99", "Presbítero": "6,99",
    "Líder": "6,99", "Levita": "3,99", "Membro": "3,99"
  });
  const [checkoutUrls, setCheckoutUrls] = useState<any>({});

  const getRolePrice = (role: string) => {
    return prices[role] || "6,99";
  };

  const getRoleCheckoutUrl = (role: string) => {
    return checkoutUrls[role] || null;
  };

  useEffect(() => {
    if (isOpen && user?.id) {
      const fetchData = async () => {
        // 1. Busca Configurações de Preço Reais do Dashboard
        ...
        if (config?.value) {
          const priceMap: any = {};
          const urlMap: any = {};
          config.value.forEach((group: any) => { 
            // Novo formato de grupos
            if (group.roles) {
              group.roles.forEach((r: string) => {
                priceMap[r] = group.price;
                urlMap[r] = group.checkout_url;
              });
            } else if (group.role) {
              // Fallback para formato antigo
              priceMap[group.role] = group.price;
            }
          });
          setPrices(priceMap);
          setCheckoutUrls(urlMap);
        }

        // 2. Busca Solicitação Existente
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 1. Upload ID
      const idUrl = await uploadMedia(idFile, { bucket: 'verifications', folder: 'ids', compressImages: false });
      if (!idUrl) throw new Error("Falha no upload do RG/CNH.");

      // 2. Upload Credencial
      let ministerialUrl = null;
      if (ministerialFile) {
        ministerialUrl = await uploadMedia(ministerialFile, { bucket: 'verifications', folder: 'credentials', compressImages: false });
        if (!ministerialUrl) throw new Error("Falha no upload da credencial.");
      }

      // 3. Upload PIX
      const pixUrl = await uploadMedia(paymentProof, { bucket: 'verifications', folder: 'receipts', compressImages: false });
      if (!pixUrl) throw new Error("Falha no upload do comprovante.");
          ...
          <div className="p-6">
            ...
            {step === 1 && (
              <div className="space-y-6">
                 ...
                 <button 
                   disabled={!selectedRole || existingRequest?.status === 'pending' || (existingRequest?.status === 'approved' && existingRequest?.requested_role === selectedRole)}
                   onClick={() => {
                     const url = getRoleCheckoutUrl(selectedRole);
                     if (!url) {
                       toast.error("O Administrador ainda não configurou o link de pagamento para este cargo.");
                       return;
                     }
                     // Redireciona para o Kiwify injetando o user_id e o cargo
                     window.location.href = `${url}?src=${user.id}_${selectedRole}`;
                   }}
            ...
            {step === 3 && (
              <div className="space-y-6">
                 ...
                 <button 
                  ...
                  className="w-full mt-8 bg-whatsapp-teal text-white py-4 rounded-xl font-bold uppercase tracking-widest text-sm hover:bg-whatsapp-tealLight active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {(loading || isUploading) ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : "Enviar Solicitação Completa"}
                </button>
              </div>
            )}
          </div>
