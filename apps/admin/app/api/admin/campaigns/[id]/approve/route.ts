 ...
 *
 * Admin aprova campanha → transação atômica no banco:
 *   lock(wallet) → valida saldo → debita → ledger → status = ativa
 *
 * Respostas:
 *   200 — campanha ativa
 *   402 — saldo insuficiente (parceiro precisa recarregar)
 *   422 — campanha não está em status aprovável
...
export async function POST(
  ...
  try {
    ...
    const campaign = await service.approveCampaign(params.id, admin.id);

    return NextResponse.json({
      ...campaign,
      mensagem: "Campanha aprovada com sucesso. Saldo debitado e campanha ativada.",
    });
  } catch (error: unknown) {
    ...
    return handleApiError(error, `POST /api/admin/campaigns/${params.id}/approve`);
  }
}
