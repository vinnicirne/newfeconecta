  ...
  async function fetchPosts() {
    try {
      ...
      const { data: postsData, error: postsError } = await supabase
        .from('posts')
        .select(`id, content, post_type, created_at, author_id, media_url, media_type, background`)
            ...
            <div className="flex-1 bg-black/5 dark:bg-[#111] flex items-center justify-center overflow-hidden min-h-[300px] relative">
              ...
              {previewPost.post_type === 'video' && previewPost.media_url && (
                <div className="w-full h-full flex items-center justify-center">
                  ...
                  {(() => {
                    ...
                    return <video src={previewPost.media_url} crossOrigin="anonymous" controls className="w-full h-full" autoPlay />;
                  })()}
                </div>
              )}
              {!previewPost.media_url && (
                previewPost.background ? (
                  ...
                  >
                    <p className="text-white text-xl font-black whitespace-pre-wrap drop-shadow-md leading-relaxed">
                      {previewPost.content}
                    </p>
                  </div>
                ) : (
                          ...
                          {post.media_url ? (
                            post.post_type === 'video' ? (
                               <div className="relative w-full h-full">
                                 <video src={post.media_url} crossOrigin="anonymous" className="w-full h-full object-cover opacity-50" />
