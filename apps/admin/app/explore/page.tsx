...
export default function ExplorePage() {
  ...
  return (
    <div className="max-w-2xl mx-auto pb-24 relative min-h-screen">
      <div className="px-4 pt-[calc(env(safe-area-inset-top)+1.5rem)] pb-2">
                ...
                <div className="space-y-4">
                  {results.profiles.map(p => (
                    <Link key={p.id} href={`/profile/${p.username}`} className="flex items-center justify-between group">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl overflow-hidden border border-gray-100 dark:border-white/10 bg-whatsapp-teal/10">
                          {p.avatar_url && !p.avatar_url.includes('vercel.sh') ? (
